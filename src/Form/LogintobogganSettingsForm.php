...
class LogintobogganSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['registration']['validate_by_url'] = array(
      '#type' => 'checkbox',
      '#title' => t('Validate by URL'),
      '#default_value' => $config->get('validate_by_url'),
      '#description' => $this->t("When set password is activated, the validation URL is invalidated.  In some
      cases you might like to keep this working."),
      '#states' => array(
        'visible' => array(
    ...
    $form['messages']['message_preauth_validate'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Pre Auth Validation'),
      '#default_value' => $config->get('message_preauth_validate'),
      '#description' => $this->t('The message displayed on the site after a user validates their email when a preauth
       role is used.'),
    );
    ...
    $form['other']['admin_email_text'] = array(
      '#type' => 'textarea',
      '#title' => $this->t('Content of admin email'),
      '#default_value' => $config->get('admin_email_text'),
      '#description' => $this->t('Configure the content of the admin email sent out when a new user needs approval.')
    );
...
//    $config = $this->configFactory->get('logintoboggan.settings');
    ...
    $config = $this->config('logintoboggan.settings');
    $user_config = \Drupal::service('config.factory')->getEditable('user.settings');
    foreach ($form_state->getValues() as $key => $value) {
      if (!in_array($key, array('submit', 'form_build_id', 'form_token', 'form_id', 'op'))) {
        if ($key == 'user_email_verification') {
          $user_config->set('verify_mail', !$value)->save();
        }
        else {
